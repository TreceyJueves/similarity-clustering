...
char sizeFile[]="Out_SliceSize_Basketball_480_Slice16_Gop32_3.log";
char answerFile[]="random.txt" ;
...
int main()
{
	...
	for(int k = 0; k < 5; k++){
		for(int i = 1; i <= 900; i++){
			bool flag = false;
		    for(int j = 0; j < bagnum; j++){
				if(bagweight[j]+w[i]<=2000 && totalbagWeight+w[i]<=totalnodeWeight+totalnodeWeight/2){
					flag = true;
					bagweight[j]+=w[i];
					baggy[j].push_back(i);
					totalbagWeight+=w[i]; 
	...
	while(totalbagWeight <= totalnodeWeight+totalnodeWeight/2){
	    ...
	    bool flag = false;
		for(int j = 0; j < bagnum; j++){
			if(bagweight[j]+w[rr]<=2000 && totalbagWeight+w[rr]<=totalnodeWeight+totalnodeWeight/2){
				flag = true;
				bagweight[j]+=w[rr];
